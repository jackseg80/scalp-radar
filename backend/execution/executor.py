...
from backend.execution import boot_reconciler as _reconciler
from backend.execution import order_monitor as _monitor
...
class Executor:
    ...
    # ─── Surveillance positions (délégué à order_monitor.py) ────────────

    async def _watch_orders_loop(self) -> None:
        await _monitor.watch_orders_loop(self)

    async def _process_watched_order(self, order: dict) -> None:
        await _monitor.process_watched_order(self, order)

    async def _poll_positions_loop(self) -> None:
        await _monitor.poll_positions_loop(self)

    async def _check_position_still_open(self, symbol: str) -> None:
        await _monitor.check_position_still_open(self, symbol)

    async def _check_grid_still_open(self, symbol: str) -> None:
        await _monitor.check_grid_still_open(self, symbol)

    async def _handle_exchange_close(
        self, symbol: str, exit_price: float, exit_reason: str,
        exit_fee: float | None = None,
    ) -> None:
        await _monitor.handle_exchange_close(self, symbol, exit_price, exit_reason, exit_fee)

    # ─── Réconciliation au boot (délégué à boot_reconciler.py) ──────────

    async def _reconcile_on_boot(self) -> None:
        await _reconciler.reconcile_on_boot(self)

    async def _reconcile_symbol(self, futures_sym: str) -> None:
        await _reconciler._reconcile_symbol(self, futures_sym)

    async def _reconcile_grid_symbol(self, futures_sym: str) -> None:
        await _reconciler._reconcile_grid_symbol(self, futures_sym)

    async def _cancel_orphan_orders(self) -> None:
        await _reconciler.cancel_orphan_orders(self)
