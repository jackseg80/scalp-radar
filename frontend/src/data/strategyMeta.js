...
const STRATEGIES = [
  // ─── Grid / DCA (8) ───
  {
    ...
    status: 'live',
    hasGuide: true,
    shortDesc: 'Enveloppes adaptatives SMA ± ATR × multiplier avec plancher anti-bruit. Achète les dips sous la SMA, revend au retour.',
    edge: "L'ATR rend la grille adaptative à la volatilité. Le plancher min_grid_spacing empêche les trades en micro-volatilité où les fees mangent le profit.",
    strengths: [
      'Excelle en crash + recovery',
      'Grille auto-adaptative à la volatilité',
      'Plancher anti-bruit en basse volatilité',
      'Validé sur 13 assets (6 Grade A, 7 Grade B)',
    ],
    weaknesses: [
      'Inactif en marché calme si ATR trop bas',
      'Vulnérable aux tendances baissières prolongées sans recovery',
    ],
    keyParams: [
      { name: 'ma_period', default: '7-20', desc: 'Période SMA (moyenne mobile)' },
      { name: 'atr_period', default: '10-20', desc: "Période ATR (mesure de volatilité)" },
      { name: 'atr_multiplier_start', default: '1.0-3.0', desc: 'Multiplier ATR du 1er niveau' },
      { name: 'atr_multiplier_step', default: '0.5-1.5', desc: "Incrément par niveau supplémentaire" },
      { name: 'num_levels', default: '3-4', desc: 'Nombre de niveaux DCA' },
      { name: 'sl_percent', default: '15-25', desc: 'Stop Loss global (% du prix moyen)' },
      { name: 'min_grid_spacing_pct', default: '0-1.8', desc: 'Plancher espacement grille (% du prix). Empêche les grilles microscopiques en basse vol.' },
      { name: 'min_profit_pct', default: '0-0.4', desc: 'Profit minimum pour le TP (%). Le TP ne fire que si le profit couvre les fees.' },
    ],
    entryLogic: 'effective_atr = max(ATR, prix × min_grid_spacing_pct%). entry = SMA - effective_atr × (start + level × step). Un seul côté actif à la fois.',
    exitLogic: 'TP = prix revient à la SMA ET profit >= min_profit_pct%. SL = prix moyen ± sl_percent%.',
    wfoGrade: 'A',
  },
  {
    id: 'grid_boltrend',
