...
### Cleanup Frontend — Heatmap + RiskCalc (20 février 2026)

Suppression de deux pages frontend obsolètes sans valeur ajoutée.

**Supprimé :**

- `Heatmap.jsx` — matrice assets×stratégies qui référençait les 4 stratégies scalp 5m (vwap_rsi, momentum, funding, liquidation), désactivées depuis longtemps. Page sans données utiles.
- `RiskCalc.jsx` — calculateur de risque statique déconnecté du système réel (leverage, capital, SL calculés manuellement). Redondant avec RiskCalc intégré au Scanner.
- Endpoint `GET /api/signals/matrix` dans `conditions_routes.py` — exclusivement utilisé par Heatmap.jsx.
- Sections CSS `.heatmap-grid`, `.heatmap-cell`, `.heatmap-header`, `.risk-input`, `.risk-slider` dans `styles.css`.
- Onglets `Heatmap` et `Risque` dans `App.jsx` (TABS, validTabs, imports, rendus).

**Conservé :**

- `GET /api/simulator/conditions` — encore utilisé par Scanner.jsx.
- `HeatmapChart.jsx` — composant SVG utilisé par ExplorerPage (heatmaps WFO), différent de Heatmap.jsx.
- Styles heatmap dans `ExplorerPage.css` (heatmap-container, heatmap-header, heatmap-subtitle).

**Tests** : 1497 passants, 0 régression. Build frontend OK (-398 lignes supprimées).
...
### Sprint Strategy Lab V3 — Comparateur Envelope DCA vs Grid ATR + Sliders BolTrend (21 février 2026)

**But** : Enrichir le Strategy Lab avec un comparateur visuel montrant pourquoi grid_atr a remplacé envelope_dca, et ajouter des sliders de paramètres au tutoriel Grid BolTrend.

**Feature 1 — ComparatorGuide.jsx** (nouveau fichier) :
- Comparateur interactif **6 étapes** avec **2 graphiques Recharts côte à côte** (lazy-loaded)
- Mêmes données BTC-like (18 points, ATR 450→640→510), domaine Y-axis partagé
- Calcul dynamique des niveaux : `computeEnvelopeLevels()` (% fixes) vs `computeGridAtrLevels()` (ATR adaptatif)
- Étapes : Intro → Marché calme → Crash (ATR explose) → DCA (prix différents) → TP (gains comparés) → Verdict (tableau récapitulatif)
- Panneau chiffres à deux colonnes (Envelope DCA / Grid ATR) avec prix moyens et niveaux
- Key insights dynamiques (étapes 4-5 : avg, gains calculés à partir des données)

**Feature 2 — Sliders dans GridBolTrendGuide.jsx** (refactorisé) :
- 4 sliders : `bol_std` (1.0-3.5), `atr_spacing_mult` (0.5-3.0), `num_levels` (1-5), `sl_percent` (5-30%)
- `bol_std` recalcule les bandes Bollinger via `recalcBands()` (déduit sigma des données base)
- Breakout détecté dynamiquement (`findBreakoutIndex()`) — si `bol_std` trop grand, message "Pas de breakout détecté"
- Panneau collapsible identique à GridAtrGuide + bouton Reset
- Support dynamique `numLevels` (1-5) et `buys: 'all'` dans STEPS

**Fichiers créés** (1) :
- `frontend/src/components/guides/ComparatorGuide.jsx` — comparateur 6 étapes

**Fichiers modifiés** (3) :
- `frontend/src/components/guides/GridBolTrendGuide.jsx` — refonte complète (constantes → useState, sliders, breakout dynamique)
- `frontend/src/components/StrategyDetail.jsx` — ajout `envelope_dca` dans `guideComponents` lazy map
- `frontend/src/data/strategyMeta.js` — `hasGuide: true` pour envelope_dca
...
- **Sprint 37 Timeframe Coherence Guard** : `--force-timeframe`, `--symbols`, `--exclude`, `--ignore-tf-conflicts` dans `scripts/optimize.py` ; `TimeframeConflictError` dans `portfolio_engine.py` (HTTP 409 sur `/apply`, catch CLI) ; badge TF orange dans ResearchPage + modale conflit 409 ; 11 tests
...
- **Sprint 38 Shallow Validation Penalty** : pénalité `n_windows` sur le score brut du grading (≥24 → 0, 18-23 → -10, 12-17 → -20, <12 → -25), `GradeResult` dataclass (grade/score/is_shallow/raw_score), `--regrade` CLI (recalcule grades is_latest sans relancer WFO), badge ⚠ shallow frontend ResearchPage, contrainte SL max 25% dans `param_grids.yaml` (grid_atr), 13 tests. **Bugs fixés** : `monte_carlo_pvalue = 0.0 or 1.0` → falsy (fix : `if x is not None else 1.0`) ; `old_score` float vs format `%d` (fix : `int(old_score)`). **Analyses** : régression rendement +74.8% → +43.6% après WFO re-run post-Hotfix 37c (fix régimes expose les fenêtres crash → params plus conservateurs, SL moyen 22.1% → 25.5%) ; 10/21 assets plafonnaient à SL=30% → contrainte 25% réduit worst SL 38.9% → 31.8% ; audit combo_score 4 variantes (V1/V3 = 20/21 assets identiques → scoring non responsable de la régression, cause = données WFO elles-mêmes) ; `scripts/audit_combo_score.py` créé (lecture seule, rapport JSON dans `data/analysis/`) ; diagnostic margin starvation invalidé (0 trades skippés, margin moyenne 1.5%). **Décisions** : SL max 25% déployé, leverage 7x maintenu (ratio rendement/DD 1.65 > 1.54 à 6x), ICP et OP exclus (Grade C shallow), 19 assets déployés (12 Grade A + 7 Grade B). **Métriques finales** (portfolio backtest 365j, 7x, SL≤25%) : Return +43.6%, Max DD -26.4%, Win rate 74.3%, 5949 trades, 0 kill switch, 15/19 assets profitables
...
- **Sprint 40 WFO Robustesse** — 3 biais méthodologiques corrigés (Sprint 40a) + 2 améliorations défensives (Sprint 40b) :
  - **#1 Taker fee sur tous les exits** : TP dans `_simulate_grid_common` et `_simulate_grid_range` utilisaient `maker_fee` (0.02%) ; en live l'executor envoie un market order → taker (0.06%). +23 tests (taker_all_exits, parité bitwise mise à jour)
  - **#2 `max_margin_ratio` dans `BacktestConfig`** : champ informatif 0.70 propagé dans `bt_config_dict`. Garde cumulatif retiré (incompatible mono-asset). +5 tests (margin_guard_wfo)
  - **#3 Embargo IS→OOS 7j** : `embargo_days` dans `_build_windows()` (défaut=0, rétrocompat), propagé depuis `param_grids.yaml` (grid_atr/boltrend/multi_tf). +12 tests (wfo_embargo)
...
- **Sprint 36b Multi-Executor** (23 fév 2026) — un Executor par stratégie live avec isolation financière complète :
  - **Architecture** : `ExecutorManager` agrégeant N `Executor`, duck-type l'ancien singleton (frontend/WS inchangés)
  - **Clés per-strategy** : `BITGET_API_KEY_{STRATEGY_UPPER}` avec fallback sur les clés globales ; `get_executor_keys()` + `has_dedicated_keys()` dans `AppConfig`
  - **Isolation** : `Executor(strategy_name=...)` filtre ses stratégies dans `set_strategies()`, ses assets dans `start()`, utilise ses propres clés dans `_create_exchange()`
  - **State persistence** : `data/executor_{strategy_name}_state.json` avec migration legacy (`executor_state.json`) au 1er boot
  - **API** : `GET /api/executor/status?strategy=grid_atr` individuel, kill-switch/reset par stratégie, ordres mergés
  - **Watchdog** : itère tous les executors du `ExecutorManager` (tag `[grid_atr]` dans les issues)
  - **Warning boot** : si >1 executor partage les mêmes clés globales → rate limit Bitget potentiel
...
- **Sprint 45 Live Journal** (25 fév 2026) — Séparation Live/Paper dans le journal + sync Bitget trades :
  - **Architecture** : table `live_trades` en DB (persiste les trades Bitget), script `sync_bitget_trades.py` (classification auto + P&L FIFO), onglet Live dédié dans JournalPage
  - **Sprint 45** : séparation onglets Live/Paper, endpoints `/api/journal/live-trades`, `/live-stats`, `/live-daily-pnl`, `/live-per-asset`, `/live-count`, `insert_live_trade()` dans Executor + DB
  - **Sprint 45 hotfix** : `sync_bitget_trades.py` — classification trades (DCA entry, cycle_close, tp_close, sl_close), P&L FIFO (appariement sells→buys par timestamp), `config.strategies` Pydantic fix
  - **Sprint 45b** : max_drawdown N/A frontend (affiche "N/A" au lieu de "-null%"), agrégation fills Bitget (win rate correct)
  - **Sprint 45c** : grouper les closes en cycles de grille (cycle complet = entrées + close associée), P&L par cycle
  - **Sprint 45d** : fusion closes en rafale (burst < 2s → 1 cycle), filtre dust notionnel (< 1$ ignoré)
  - **Fichiers** : `backend/core/database.py`, `backend/execution/executor.py`, `backend/api/journal_routes.py`, `scripts/sync_bitget_trades.py`, `frontend/src/components/JournalPage.jsx`, `tests/test_live_trades.py`
  - **41 nouveaux tests** → **1891 tests** (+10 misc)
- **Sprint 46 Journal Améliorations** (25 fév 2026) — P&L Jour, filtre stratégie Live, balance snapshots :
  - **Fix 1 — P&L Jour** : remplace "P&L Session" (reset au deploy) par "P&L Jour" + "P&L Total" dans ExecutorPanel, tooltip "depuis XX/02/2026", endpoint `GET /api/journal/daily-pnl-summary`, méthode `get_daily_pnl_summary()` en DB
  - **Fix 2 — Filtre stratégie** : dropdown dans l'onglet Live journal (auto-détection des stratégies depuis live-stats), propagation `?strategy=` aux composants enfants (LiveStatsOverview, LiveDailyPnl, LivePerAssetSummary)
  - **Fix 3 — Balance snapshots** : table `balance_snapshots` (timestamp, strategy_name, balance, unrealized_pnl, margin_used, equity), persistence best-effort toutes les ~1h (piggybacked sur `_balance_refresh_loop`), endpoint `GET /api/journal/live-equity`, composant `LiveEquityCurve` (SVG)
...
- **Sprint 47d — Audit & déploiement grid_atr V2** (25 fév 2026) — Vérification de bout en bout de la chaîne grid_atr V2 et correction des gaps de propagation per_asset :
  - **Gap corrigé (principal)** : le `GridStrategyRunner` (paper) et l'`Executor` (live) ne patchaient pas `min_grid_spacing_pct` avant `compute_grid()` ni `min_profit_pct` avant `should_close_all()`. Les per_asset overrides de `strategies.yaml` étaient ignorés — tous les assets utilisaient `0.0` (valeur top-level).
  - **simulator.py** : ajout helper `_get_per_asset_float(symbol, param, default)` + patch `min_grid_spacing_pct` dans le bloc `compute_grid()` + patch `min_profit_pct` autour de `should_close_all()`
  - **executor.py** : même patches aux deux call sites (`compute_grid()` et `should_close_all()`), helper statique `_get_per_asset_float(strategy, symbol, param, default)`
  - **grid_atr.py** : log informatif "plancher ATR actif" une fois par nouveau cycle (`not grid_state.positions`), avec effective_atr, raw_atr et pourcentages
...
- **Sprint 49 — Rapport Telegram Hebdomadaire** (25 fév 2026) — Résumé de performance automatique envoyé chaque lundi à 08:00 UTC :
  - **`WeeklyReporter`** : classe dans `backend/alerts/weekly_reporter.py` suivant le pattern `Heartbeat`. Calcule le prochain lundi 08:00 UTC et sleep jusqu'alors (correct après restart en milieu de semaine). Collecte les données en parallèle via `asyncio.gather()`.
  - **Format** : section GLOBAL (solde total, P&L semaine + %, P&L total, trades + WR) + une section par stratégie (⚡ LIVE avec balance/leverage/max DD/top-worst assets, 👁️ PAPER avec P&L seulement) + uptime calculé depuis les `balance_snapshots`.
  - **Sources** : stratégies LIVE → méthodes DB existantes (`get_live_stats`, `get_live_per_asset_stats`, `get_daily_pnl_summary`, `get_max_drawdown_from_snapshots`, `get_balance_snapshots`) ; stratégies PAPER → SQL direct sur `simulation_trades`.
  - **Détection stratégies** : `_classify_strategies(config)` itère `config.strategies.model_fields` → (live_eligible=True → LIVE, sinon PAPER).
  - **CLI** : `scripts/weekly_report.py` avec `--dry-run` (affiche sans envoyer) et mode envoi réel.
...
- **Sprint 50a-bis — Optimisation Paramètres ema_atr** (26 fév 2026) — Sprint d'optimisation grid search ciblé sur `ema_atr` pour trouver les meilleurs paramètres en mode binaire (normal/defensive) :
  - **Résultat** : meilleurs paramètres `ema_fast=50, ema_slow=200, atr_fast=7, atr_slow=30, atr_stress_ratio=2.0, h_down=6, h_up=24`
  - **Métriques** : F1 defensive = **0.668**, F1 normal = 0.935, accuracy = 88.7%, **1.6 transitions/an** (très stable)
  - **Biais bear market évité** : hysteresis asymétrique `h_down=6` (24h) pour signaux rapides bear, `h_up=24` (96h) pour confirmations bull
  - **Décision** : `ema_atr` validé pour intégration Sprint 50b — paramètres figés dans `DEFAULT_PARAMS`
  - **Plan archivé** : `docs/plans/sprint-50a-regime-detector.md`
- **Sprint 50b — Impact Portfolio : Leverage Dynamique** (26 fév 2026) — Intégration du détecteur `ema_atr` dans le portfolio backtest pour mesurer l'impact du leverage 7x/4x dynamique sur grid_atr :
  - **RegimeSignal** : dataclass pré-calculée avec timestamps 4h, régimes binaires, transitions, lookup bisect O(log n)
  - **Leverage dynamique** : 7x en mode normal, 4x en mode defensive — changement différé si position ouverte, init au démarrage selon régime
  - **3 emplacements mis à jour** : `runner._leverage`, `runner._gpm._config.leverage`, `runner._strategy._config.leverage`
  - **CLI** : `--regime`, `--regime-normal`, `--regime-defensive` dans `portfolio_backtest.py`
  - **Script comparaison** : `regime_backtest_compare.py` — 3 runs (A: 7x fixe, B: 4x fixe, C: dynamique) + Sharpe/Calmar + verdict Go/No-Go (2/3 critères)
  - **Verdict Go/No-Go** : Return C ≥ 80% Return A + DD réduit + Sharpe amélioré
  - **Backward compatible** : sans `--regime`, comportement identique
  - **Fichiers créés** : `backend/regime/__init__.py`, `backend/regime/detectors.py`, `backend/regime/btc_regime_signal.py`, `scripts/regime_backtest_compare.py`, `tests/test_regime_signal.py`, `docs/plans/sprint-50b-regime-portfolio.md`
  - **Fichiers modifiés** : `backend/backtesting/portfolio_engine.py`, `scripts/portfolio_backtest.py`, `COMMANDS.md` (section 23 ajoutée)
  - **20 nouveaux tests** → **2006 tests, 2005 passants**, 0 régression
- **Hotfix grid_multi_tf — build_context sans compute_live_indicators** (26 fév 2026) — Bug d'intégration : grid_multi_tf n'a jamais fait de trade depuis son déploiement (0 trades live + paper confirmés) :
  - **Cause racine** : `GridStrategyRunner.build_context()` (utilisé par l'Executor via `get_runner_context()` comme source unique d'indicateurs) ne appelait jamais `compute_live_indicators()`. Résultat : `ctx.indicators["4h"]["st_direction"]` était toujours absent → `compute_grid()` retournait `[]` à chaque candle → 0 entrées possibles
  - **Fix 1 — build_context()** (`simulator.py:1331`) : ajout de l'appel à `compute_live_indicators()` après le merge SMA, identique au pattern de `on_candle()`. Inclut le même `try/except` + `isinstance(buffers, dict)` guard. Impacte tous les `GridStrategyRunner` (mais les stratégies sans override retournent `{}` → no-op)
  - **Fix 2 — on_candle() warm-up guard** (`simulator.py:948`) : suppression du `if not self._is_warming_up` redondant. Toutes les implémentations de `compute_live_indicators()` ont déjà leur propre `min_candles` check et retournent `{}` proprement si données insuffisantes
  - **Sécurité confirmée** : `base_grid.py` retourne `{}` (grid_atr inchangé), grid_multi_tf/boltrend/momentum/trend ont tous un guard `len(candles) < min_needed` qui retourne `{}` — aucun risque de NaN ou crash
  - **Diagnostic** : script `scripts/diag_grid_multi_tf.py` (supprimé après usage) — 21/21 assets avec ST valide, 34 levels < 2% du prix → preuve que le problème était l'absence de données ST dans le contexte, pas la stratégie elle-même
  - **Fichiers modifiés** : `backend/backtesting/simulator.py` (2 emplacements)
  - **Tests** : 167 tests simulator/grid passent, 0 régression — **2024 tests, 2023 passants**
- **Sprint 51 — Cooldown Fast Engine + Hotfix Fees Polling** (26 fév 2026) — Parité cooldown anti-churn entre fast backtest engine et live/paper + complétion Hotfix 34 :
  - **Cooldown `can_open_new` flag** : implémenté dans `_simulate_grid_common()`, `_simulate_grid_boltrend()`, `_simulate_grid_range()` — utilise un flag au lieu de `continue` pour ne pas bloquer les exits/trailing
  - **Propagation wrappers** : `_simulate_grid_atr`, `_simulate_envelope_dca`, `_simulate_grid_multi_tf`, `_simulate_grid_trend` passent `cooldown_candles` à `_simulate_grid_common`
  - **Warning supprimé** : plus de UserWarning "cooldown non implémenté" dans `run_multi_backtest_from_cache`
  - **Hotfix 34 complété** : `_check_position_still_open` et `_check_grid_still_open` extraient les fees réelles via `_fetch_fill_price` avec try/except fallback
...
- **Sprint 53 — Multi-Engine Resampling 4h/1d + Logging Windows Fix** (26 fév 2026) :
  - **Bug WFO Phases 2+3** : `multi_engine.run()` cherchait `candles_by_tf["4h"]` introuvable → 0 trades → score Bitget 0/15 pour tout combo 4h/1d. Fix : resampling automatique 1h→4h/1d via `resample_candles()` existant, ajustement `main_tf` au TF natif de la stratégie. `grid_multi_tf` non affecté (son TF natif est "1h"). Aucune modification de `optimize.py`, `report.py`, `walk_forward.py`.
  - **Bug Loguru Windows** : `enqueue=True` ne suffisait pas — `os.rename()` échoue lors de la rotation quand l'antivirus/indexeur verrouille le fichier → milliers d'erreurs PermissionError sur stderr. Fix : `{time}` dans le nom de fichier sur Windows → Loguru crée un nouveau fichier sans rename. Compression désactivée sur Windows (même cause). Comportement Linux prod inchangé.
  - **ARB/USDT retiré** de `config/assets.yaml` (volume faible), tests de config mis à jour (20 assets)
...
- **Sprint 55 — Renforcement réalisme fast engine WFO** (27 fév 2026) :
  - **Audit WFO** : 2 failles critiques identifiées — (1) combos sélectionnés avec SL×leverage > 150% incompatibles avec le trading réel en cross margin ; (2) aucun kill switch dans le fast engine alors que le live stoppe à 25% DD par runner.
  - **Fix 1 — Filtre SL×leverage** (`walk_forward.py`) : `_filter_sl_leverage(grid, leverage, threshold=1.5, min_combos=50)` — filtre les combos où `sl_percent / 100 × leverage > 1.5` (ex : SL 25% × 7x = 1.75 > 1.5 → exclu). Fallback par tri SL croissant si trop de combos filtrés. Appliqué sur `full_grid` ET `fine_grid` (±1 step peut créer des combos invalides).
  - **Fix 2 — Kill switch 25% DD** (`fast_multi_backtest.py`, `fast_backtest.py`) : `KILL_SWITCH_DD_PCT = 0.25` — cohérent avec `grid_max_session_loss_percent: 25%` en live (seuil per-runner, pas le global 45%). Implémenté dans 5 fonctions grid (`_simulate_grid_common`, `_simulate_grid_range`, `_simulate_grid_funding`, `_simulate_grid_boltrend`, `_simulate_grid_momentum`) + scalp `_simulate_trades()` Python fallback. Les 6 fonctions numba JIT reportées Sprint 56 (strategies live = grid, scalp = paper).
  - **Rationale 25% vs 45%** : le WFO simule 1 asset = 1 runner. En live, c'est le seuil per-runner (25%) qui stoppe ce runner, pas le global (45%) qui est pour le portfolio. Utiliser 45% laisserait passer des combos tués à -25% en live → incohérence.
  - **Adaptations tests** : `test_dd_guard.py` (guard 80%→15% pour différencier du kill switch 25%) ; `test_fast_engine_refactor.py` (valeurs parity grid_atr mises à jour : 108→57 trades) ; `test_grid_atr.py` et `test_grid_multi_tf.py` (parity fast vs normal assouplie — fast engine a un kill switch que le normal engine n'a pas).
...
- **Sprint 62a — Guard timeframe dans `--apply`** (1 mars 2026) : bug WFO multi-TF — grille `grid_atr` avec `timeframe: [1h, 4h, 1d]` sélectionnait `tf=1d` comme best IS combo, écrasant les bons résultats 1h (BTC: Grade B→D, SOL: Grade A→F). Fix : `apply_from_db()` lit le `timeframe` de référence depuis `strategies.yaml`, filtre les résultats `is_latest=1` dont le TF ≠ référence (WARNING + ignore), fallback mode (TF le plus fréquent parmi grades A/B) si pas de TF dans yaml (tiebreak via `TF_ORDER`). Blocage Sprint 37 supplanté par le guard (plus jamais atteint). **Fichiers** : `scripts/optimize.py` (guard section 0 dans `apply_from_db`), `tests/test_timeframe_coherence.py` (+3 nouveaux, 2 mis à jour), `tests/test_optimization.py` (fix glob backup `old/`). **3 nouveaux tests, 1 test pré-existant corrigé** → **2165 tests, 2159 passants** (6 pré-existants), 0 régression
- **Sprint 62b — Filtre ATR minimum pour `grid_atr`** (1 mars 2026) : nouveau paramètre `min_atr_pct` (float, défaut 0.0 = désactivé). Si `ATR/close × 100 < min_atr_pct`, skip ouverture de nouveaux niveaux ; les exits (TP, SL) des positions existantes restent actifs (filtre en section 4b, après section 1 exits). Grille WFO : `[0.0, 0.5, 1.0, 1.5]` (864 → 3456 combos). **Fichiers** : `backend/core/config.py` (`GridATRConfig.min_atr_pct`), `backend/strategies/grid_atr.py` (filtre dans `compute_grid()` + `get_params()`), `backend/optimization/fast_multi_backtest.py` (paramètres `min_atr_pct`/`atr_arr_for_filter` dans `_simulate_grid_common`, passage depuis `_simulate_grid_atr`), `config/param_grids.yaml` (`grid_atr.default.min_atr_pct`), `tests/test_grid_atr_min_atr.py` (nouveau, 7 tests). **7 nouveaux tests** → **2172 tests, 2166 passants** (6 pré-existants), 0 régression
- **Sprint 63a — Overview Dashboard + Equity Live Enrichie + Drawdown Chart** (1 mars 2026) : refonte des 3 composants visuels les plus visibles du dashboard. (1) `OverviewPage` → dashboard d'accueil enrichi : 4 KPI cards (Capital, P&L Total, Win Rate, Max DD) avec couleurs conditionnelles, `EnhancedEquityCurve` pleine largeur + `DrawdownChart` sous l'equity, tableau Top 3 / Bottom 3 assets par P&L, régime BTC inline, 3 derniers événements journal, strategies + positions en `CollapsibleCard` collapsées. (2) `EnhancedEquityCurve` → Recharts `AreaChart` avec gradient fill vert/rouge, overlay de bandes de régime colorées (`ReferenceArea`), ligne de référence capital initial, selector de période 7j/30j/90j/Tout, tooltip enrichi (date, equity, P&L jour, ratio marge). (3) `DrawdownChart` → réécrit de SVG pur vers Recharts, mode autonome (fetch ses propres données via `useApi`) + mode legacy backward-compatible (`curves` pour PortfolioPage inchangé), kill switch à -45% en pointillé rouge. **Fichiers créés** : `EnhancedEquityCurve.jsx`, `EnhancedEquityCurve.css`, `DrawdownChart.css`, `OverviewPage.css`. **Fichiers modifiés** : `OverviewPage.jsx` (refonte complète), `DrawdownChart.jsx` (SVG→Recharts), `JournalPage.jsx` (LiveEquityCurve inline supprimée → EnhancedEquityCurve + DrawdownChart). **0 nouveau test** (frontend pur) → **2172 tests, 2166 passants**, 0 régression
...
- The first attempted `grid_boltrend` snapshot (`snapshot-5a23b693c8fb56af`) was correctly `INVALID` before WFO: Bitget 1m ended on 2026-02-21, seven symbols were absent, and several Binance 1h rows had one-bar gaps. Bitget UTA v3 long-history collection and explicit cutoff-coverage validation were added; the snapshot must be recreated from a clean worktree after data repair.
